...
#'   requests.
#' @param force Whether to update Clio token even when there is an existing
#' token.
#' @export
...
clio_token <- function(force=FALSE) {
  ...
  if (isFALSE(force) &&
      ...
      payload$email != getOption("malevnc.clio_email"))
...
clio_fetch <- function(url, config=NULL, ..., body=NULL, query=NULL, json=FALSE,
                       app='natverse') {
  if (!is.null(app)) query <- c(query, list(app = app))
  if (is.null(config))
